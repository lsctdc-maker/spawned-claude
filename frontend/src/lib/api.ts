...
export async function extractUSPs(
  ...
  try {
    const response = await apiClient.post('/extract-usps', {
      productInfo,
    ...
    // 개발 모드: 백엔드 없이 테스트용 모의 데이터
    console.warn('API 호출 실패, 모의 데이터를 사용합니다:', error);
    return {
      success: true,
      data: {
        usps: generateMockUSPs(productInfo),
      },
    };
  }
}
...
export async function generateSections(
  ...
  try {
    const response = await apiClient.post('/generate-sections', {
      productInfo,
      usps,
      tone,
    });
    ...
    return {
      success: true,
      data: {
        sections: generateMockSections(productInfo, usps, tone),
      },
    };
  }
}
...
export async function exportHTML(
  ...
  try {
    ...
    console.warn('API 호출 실패, 로컬 생성을 사용합니다:', error);
    return {
      success: true,
      data: {
        html: buildLocalHTML(sections, productInfo),
      },
    };
  }
}
    ...
    },
    {
      ...
      content: {
        paragraphs: [
          {
            title: `왜 ${name}인가요?`,
            text: `${name}은(는) 고객의 니즈를 깊이 연구하여 탄생한 제품입니다. 최고급 소재와 정교한 기술이 만나 완벽한 결과물을 만들어냅니다.`,
            imagePosition: 'right' as const,
          },
          {
            title: '세심한 디테일',
            text: '모든 공정에서 엄격한 품질 관리를 거치며, 고객 만족을 최우선으로 합니다. 작은 디테일 하나까지 놓치지 않았습니다.',
