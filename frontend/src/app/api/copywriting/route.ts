...
function buildSectionsFromAI(
  ...
  if (data.hero) {
    ...
    sections.push({
      ...
      content: {
        ...
        subheadline: hero.subheadline || productInfo.shortDescription,
        ctaText: hero.ctaText || '자세히 보기',
        backgroundStyle: (hero.backgroundStyle as 'gradient' | 'solid' | 'image') || 'gradient',
      },
    });
  }

  if (data.usp_points) {
    const uspPoints = data.usp_points as { points?: Array<{ id?: string; title: string; description: string; icon?: string }> };
    sections.push({
      id: 'section-usp', type: 'usp', title: 'USP 포인트', visible: true, order: 1,
      content: {
        points: (uspPoints.points || usps).map((p, i) => ({
          id: p.id || `usp-ai-${i}`,
          title: p.title,
  ...
  if (data.detail) {
    const detail = data.detail as { paragraphs?: Array<{ title: string; text: string; imagePosition?: string }> };
    sections.push({
      ...
      content: {
        paragraphs: (detail.paragraphs || []).map((p) => ({
          title: p.title,
          text: p.text,
          imagePosition: (p.imagePosition as 'left' | 'right') || 'right',
        })),
      },
    });
  }
  ...
  if (data.comparison) {
    const comparison = data.comparison as { headers?: string[]; rows?: Array<{ label: string; values: string[] }> };
    sections.push({
      id: 'section-comparison', type: 'comparison', title: '비교표', visible: true, order: 3,
      content: {
        headers: comparison.headers || ['항목', name, '일반 제품'],
        rows: (comparison.rows || []).map((r) => ({ label: r.label, values: r.values })),
      },
    });
  }

  if (data.howto) {
    const howto = data.howto as { steps?: Array<{ step: number; title: string; description: string }> };
    sections.push({
      id: 'section-howto', type: 'howto', title: '사용법', visible: true, order: 4,
      content: {
        steps: (howto.steps || []).map((s) => ({ step: s.step, title: s.title, description: s.description })),
      },
    });
  }

  if (data.certification) {
    const cert = data.certification as { items?: Array<{ name: string; description: string; icon?: string }> };
    sections.push({
      id: 'section-certification', type: 'certification', title: '인증/신뢰', visible: true, order: 5,
      content: {
        items: (cert.items || []).map((item) => ({ name: item.name, description: item.description, icon: item.icon || '🏅' })),
      },
    });
  }

  if (data.reviews) {
    const reviews = data.reviews as { reviews?: Array<{ author: string; rating?: number; text: string; date?: string }> };
    sections.push({
      id: 'section-reviews', type: 'reviews', title: '고객 리뷰', visible: true, order: 6,
      content: {
        reviews: (reviews.reviews || []).map((r) => ({ author: r.author, rating: r.rating || 5, text: r.text, date: r.date || '2024-01-01' })),
      },
    });
  }

  if (data.faq) {
    const faq = data.faq as { items?: Array<{ question: string; answer: string }> };
    sections.push({
      id: 'section-faq', type: 'faq', title: 'FAQ', visible: true, order: 7,
      content: { items: (faq.items || []).map((item) => ({ question: item.question, answer: item.answer })) },
    });
  }

  if (data.cta) {
    const cta = data.cta as { headline?: string; subtext?: string; buttonText?: string; urgencyText?: string };
    sections.push({
      id: 'section-cta', type: 'cta', title: 'CTA', visible: true, order: 8,
      content: {
        headline: cta.headline || `${name}, 지금 시작하세요`,
        subtext: cta.subtext || '한정 수량 특별 할인 진행 중',
        buttonText: cta.buttonText || '구매하러 가기',
        urgencyText: cta.urgencyText || '',
      },
    });
  }
...
function generateFallbackSections(
  ...
  return [
    ...
    { id: 'section-usp', type: 'usp', title: 'USP 포인트', visible: true, order: 1, content: { points: usps } },
    {
      id: 'section-detail', type: 'detail', title: '제품 상세', visible: true, order: 2,
      content: {
        paragraphs: [
