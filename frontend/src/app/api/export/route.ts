...
export async function POST(request: NextRequest) {
  try {
    ...
    const { sections, productInfo } = body as {
...
function buildExportHTML(sections: DetailPageSection[], productInfo: ProductInfo): string {
...
function renderSection(section: DetailPageSection, primaryColor: string, productName: string, categoryIcon: string): string {
  switch (section.type) {
    case 'hero': return renderHero(section.content as HeroContent, primaryColor, productName);
    case 'usp': return renderUSP(section.content as USPContent, primaryColor);
    case 'detail': return renderDetail(section.content as DetailContent, primaryColor, categoryIcon);
    case 'comparison': return renderComparison(section.content as ComparisonContent, primaryColor);
    case 'howto': return renderHowTo(section.content as HowToContent, primaryColor);
    case 'certification': return renderCertification(section.content as CertificationContent);
    case 'reviews': return renderReviews(section.content as ReviewContent, primaryColor);
    case 'faq': return renderFAQ(section.content as FAQContent, primaryColor);
    case 'cta': return renderCTA(section.content as CTAContent, primaryColor);
    default: return '';
  }
}

function renderHero(c: HeroContent, color: string, _price?: string): string {
  ...
  <h1 style="font-size:36px;font-weight:800;color:${color};margin-bottom:16px;">${c.headline}</h1>
  <p style="font-size:18px;color:#555;margin-bottom:32px;max-width:500px;margin-left:auto;margin-right:auto;">${c.subheadline}</p>
  <a class="btn" style="background:${color};color:#fff;">${c.ctaText}</a>
</div>`;
}
...
function renderDetail(c: DetailContent, color: string, icon: string): string {
  const paragraphs = c.paragraphs.map((p) => `
    <div style="display:flex;gap:32px;align-items:center;margin-bottom:48px;flex-direction:${p.imagePosition === 'left' ? 'row-reverse' : 'row'};">
      <div style="flex:1;">
        <h3 style="font-size:20px;font-weight:700;margin-bottom:12px;">${p.title}</h3>
        <p style="color:#555;line-height:1.8;">${p.text}</p>
      </div>
...
function renderComparison(c: ComparisonContent, color: string): string {
  const headerCells = c.headers.map((h, i) =>
    `<th style="padding:16px 24px;text-align:center;font-weight:700;${i === 1 ? `background:${color};color:#fff;border-radius:12px 12px 0 0;` : 'background:#f9fafb;color:#374151;'}">${h}</th>`
  ).join('');
  const rows = c.rows.map((r) => {
    const cells = r.values.map((v, i) =>
      `<td style="padding:16px 24px;text-align:center;${i === 0 ? `color:${color};font-weight:700;` : 'color:#6b7280;'}">${v}</td>`
    ).join('');
    return `<tr style="border-bottom:1px solid #f3f4f6;"><td style="padding:16px 24px;font-weight:500;color:#374151;">${r.label}</td>${cells}</tr>`;
  }).join('');
  return `<div class="section">
...
function renderHowTo(c: HowToContent, color: string): string {
  const steps = c.steps.map((s, i) => `
    <div style="display:flex;gap:20px;align-items:flex-start;margin-bottom:${i < c.steps.length - 1 ? '32' : '0'}px;">
      <div style="width:40px;height:40px;border-radius:50%;background:${color};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;flex-shrink:0;">${s.step}</div>
      <div style="padding-top:4px;">
        <h3 style="font-weight:700;margin-bottom:4px;">${s.title}</h3>
        <p style="font-size:14px;color:#666;">${s.description}</p>
      </div>
    </div>`).join('');
...
function renderCertification(c: CertificationContent): string {
  const items = c.items.map((item) => `
    <div style="text-align:center;width:140px;">
      <div style="width:64px;height:64px;margin:0 auto 12px;border-radius:16px;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,0.08);display:flex;align-items:center;justify-content:center;font-size:28px;">${item.icon}</div>
      <h3 style="font-size:14px;font-weight:700;margin-bottom:4px;">${item.name}</h3>
  ...
  const reviews = c.reviews.map((r) => {
    ...
    return `<div style="padding:24px;background:#f9fafb;border-radius:16px;margin-bottom:16px;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
        <div style="width:40px;height:40px;border-radius:50%;background:${color};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;">${r.author.charAt(0)}</div>
        <div><div style="font-weight:500;font-size:14px;">${r.author}</div><div style="font-size:14px;">${stars}</div></div>
        <span style="margin-left:auto;font-size:12px;color:#aaa;">${r.date}</span>
      </div>
      <p style="font-size:14px;color:#555;">${r.text}</p>
    </div>`;
  }).join('');
  return `<div class="section">
  ...
  const items = c.items.map((item) => `
    <div style="background:#f9fafb;border-radius:16px;padding:20px 24px;margin-bottom:12px;">
      ...
      <div style="display:flex;gap:12px;align-items:flex-start;padding-top:8px;border-top:1px solid #e5e7eb;">
        ...
        <span style="font-size:14px;color:#666;">${item.answer}</span>
      </div>
    </div>`).join('');
  return `<div class="section">
...
function renderCTA(c: CTAContent, color: string): string {
  ...
  <h2 style="font-size:28px;font-weight:800;color:#fff;margin-bottom:12px;">${c.headline}</h2>
  <p style="color:rgba(255,255,255,0.8);font-size:18px;margin-bottom:32px;">${c.subtext}</p>
  <a class="btn" style="background:#fff;color:${color};font-size:18px;">${c.buttonText}</a>
  ${c.urgencyText ? `<p style="color:rgba(255,255,255,0.9);margin-top:16px;font-size:14px;font-weight:500;">${c.urgencyText}</p>` : ''}
</div>`;
}
