...
export async function composeSectionCanvas(
  ...
  // ── Phase 1: Text + shapes FIRST (sync, ~50ms) ──
  // User sees meaningful content on colored background immediately.
  ...
  for (const shapeDef of (template.useHtmlDesign ? [] : template.shapes)) {
    ...
    if (shapeDef.type === 'rect') {
      obj = new fabricModule.Rect({
        left: shapeDef.left, top: shapeDef.top,
        width: shapeDef.width || 100, height: shapeDef.height || 3,
        fill: shapeColor, rx: shapeDef.rx || 0, ry: shapeDef.ry || 0,
      ...
      obj = new fabricModule.Circle({
        left: shapeDef.left, top: shapeDef.top,
        radius: shapeDef.radius || 50, fill: shapeColor,
        opacity: shapeDef.opacity ?? 1,
        selectable: shapeDef.selectable !== false,
        evented: shapeDef.selectable !== false,
        name: shapeDef.name,
      ...
      obj = createIconObject(fabricModule, shapeDef.iconName, {
        left: shapeDef.left, top: shapeDef.top,
        size: shapeDef.iconSize || 24, color: shapeColor,
        strokeWidth: shapeDef.iconStrokeWidth, opacity: shapeDef.opacity,
      });
    } else if (shapeDef.type === 'badge') {
      obj = createBadge(fabricModule, {
        left: shapeDef.left, top: shapeDef.top,
        radius: shapeDef.radius || 20, bgColor: shapeColor,
        text: shapeDef.badgeText, iconName: shapeDef.badgeIcon,
        textColor: resolveColor(shapeDef.badgeTextColor, colors),
        fontSize: shapeDef.badgeFontSize, opacity: shapeDef.opacity,
      });
    } else if (shapeDef.type === 'gradient-rect' && shapeDef.gradient) {
      obj = createGradientRect(fabricModule, {
        left: shapeDef.left, top: shapeDef.top,
        width: shapeDef.width || 860, height: shapeDef.height || 60,
        gradient: resolveGradientColors(shapeDef.gradient, colors),
        rx: shapeDef.rx, ry: shapeDef.ry, opacity: shapeDef.opacity,
      });
    }
    if (obj) { canvas.add(obj); shapeObjs.push(obj); }
  }
  ...
  for (const textDef of template.textObjects) {
    ...
    const textObj = new fabricModule.Textbox(text, {
      left: textDef.left, top: textDef.top, width: textDef.width,
      fontSize: textDef.fontSize, fontWeight: textDef.fontWeight, fontFamily,
      fill: fillColor, textAlign: textDef.textAlign || 'left',
      lineHeight: textDef.lineHeight || 1.4,
      charSpacing: (textDef.letterSpacing || 0) * 10,
      opacity: textDef.opacity ?? 1, name: textDef.name,
      splitByGrapheme: true,
      ...(textDef.shadow ? { shadow: new fabricModule.Shadow(textDef.shadow) } : {}),
      ...(textDef.stroke ? {
        stroke: textDef.stroke, strokeWidth: textDef.strokeWidth || 0, paintFirst: 'stroke',
      } : {}),
    });
    canvas.add(textObj);
  }

  // Render Phase 1 — user sees text + shapes on colored bg
  canvas.renderAll();

  // ── Phase 2: Background + images (async, 2-5s) ──
  // Inserted behind existing objects using sendToBack.

  let bgInsertIndex = 0; // track where to insert background objects

  // Background: HTML design template OR photo image
  if (template.useHtmlDesign && template.solidBackground) {
    ...
    try {
      const designUrl = await renderDesignBackground(
        section.sectionType, template.variantId,
        { accent: colors.accent, bg: colors.bg, bg2: colors.bg2, text: colors.text },
        860, template.canvasHeight, category,
      );
      if (designUrl) {
    ...
    try {
      ...
      bgImg.set({
        ...
        left: (860 - scaledWidth) / 2, top: (template.canvasHeight - scaledHeight) / 2,
        originX: 'left', originY: 'top',
        selectable: false, evented: false, name: '배경 이미지',
      });
      if (bgImg.filters) {
  ...
  // Overlay for text readability (photo backgrounds only)
  ...
  // Product image (누끼) — insert before shapes/text
  if (template.hasProductImage && productPhotoUrl && template.productImagePosition) {
    try {
      ...
      const scale = Math.min(pos.maxWidth / prodImg.width!, pos.maxHeight / prodImg.height!);
      prodImg.set({
        left: pos.left, top: pos.top, scaleX: scale, scaleY: scale,
        name: '제품 이미지',
