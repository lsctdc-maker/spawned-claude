...
export default function PropertiesPanel({
  ...
  competitorInsights, researchStatus,
  ...
  return (
    <div className="w-64 bg-white border-l border-[#E5E8EB] flex flex-col flex-shrink-0">
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        {competitorInsights && (
          <div className="border-t border-[#E5E8EB] pt-4">
            <div className="text-[9px] uppercase tracking-widest text-[#8B95A1] mb-2">경쟁사 분석</div>
            <div className="bg-[#EBF4FF] rounded-xl p-3 mb-2">
              <div className="text-[11px] font-bold text-[#3182F6] mb-1">평균가 {competitorInsights.avgPrice?.toLocaleString()}원</div>
              <div className="text-[9px] text-[#4E5968]">
                {competitorInsights.priceRange?.min?.toLocaleString()}~{competitorInsights.priceRange?.max?.toLocaleString()}원
              </div>
            </div>
            {competitorInsights.commonKeywords?.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {competitorInsights.commonKeywords.slice(0, 8).map((kw: string, i: number) => (
                  <span key={i} className="px-2 py-0.5 text-[9px] bg-[#F4F5F7] rounded text-[#4E5968]">{kw}</span>
                ))}
              </div>
            )}
            {competitorInsights.topBrands?.length > 0 && (
              <div className="text-[9px] text-[#8B95A1]">
                상위: {competitorInsights.topBrands.slice(0, 3).join(', ')}
              </div>
            )}
          </div>
        )}
