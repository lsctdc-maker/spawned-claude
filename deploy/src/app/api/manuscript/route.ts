...
import { ProductInfo, USP, InterviewMessage, ManuscriptSection, ManuscriptSectionType } from '@/lib/types';
...
function buildPrompt(productInfo: ProductInfo, usps: USP[], interviewMessages: InterviewMessage[]): string {
  const productText = [
  ...
  "sections": [
    {
      ...
      "imageGuide": "이미지 가이드 (어떤 이미지가 필요한지 1~2문장)"
    },
    {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { productInfo, extractedUSPs, interviewMessages, productPhotoBase64, productPhotoMimeType } = body as {
    ...
    userContent.push({
      type: 'text',
      text: buildPrompt(productInfo, extractedUSPs, interviewMessages),
    });
