...
function renderComparison(c: ComparisonContent, color: string): string {
  const headerCells = c.headers.map((h, i) =>
    `<th style="padding:16px 24px;text-align:center;font-weight:700;${i === 1 ? `background:${color};color:#fff;border-radius:${12}px ${12}px 0 0;` : 'background:#f9fafb;color:#374151;'}">${h}</th>`
  ).join('');
